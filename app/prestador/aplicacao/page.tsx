  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requiredFields = [
        'fullName', 'email', 'phone', 'oabNumber', 'oabState',
        'yearsOfExperience', 'whyJoinFreeLaw', 'hoursPerWeek'
      ]
      ...
      const getExperienceLevel = (years: string) => {
        ...
        if (yearsNum < 5) return 'pleno'
        if (yearsNum < 10) return 'senior'
        return 'especialista'
      }

      // Map availability
      const getAvailability = (hours: string) => {
        const hoursNum = parseInt(hours) || 0
        if (hoursNum <= 20) return 'immediate'
        if (hoursNum <= 40) return 'week'
        return 'flexible'
      }

      // Map workload
      const getWorkload = (hours: string) => {
        const hoursNum = parseInt(hours) || 0
        if (hoursNum <= 20) return 'part_time'
        if (hoursNum <= 40) return 'full_time'
        return 'freelance'
      }

      // Map form data to our API format
      const apiData = {
        fullName: formData.fullName.trim(),
        email: formData.email.trim().toLowerCase(),
        phone: formData.phone.replace(/\D/g, ''), // Remove non-digits
        oab: formData.oabNumber.trim(),
        state: formData.oabState,
        experience: getExperienceLevel(formData.yearsOfExperience),
        specialties: formData.specialties,
        bio: `${formData.strongestSkills || 'Não informado'}\n\nExperiência: ${formData.previousExperience || 'Não informado'}\n\nAbordagem de qualidade: ${formData.qualityApproach || 'Não informado'}`,
        availability: getAvailability(formData.hoursPerWeek),
        workload: getWorkload(formData.hoursPerWeek),
        motivation: `Por que quer se juntar à Freelaw: ${formData.whyJoinFreeLaw}\n\nO que mais te empolga: ${formData.whatExcitesYou || 'Não informado'}\n\nPlanos de crescimento: ${formData.growthPlans || 'Não informado'}`,
      ...
      console.log('Aplicação enviada com sucesso:', result)
      ...
      if (result.provider) {
        ...
        localStorage.setItem('providerStatus', result.provider.status)
        localStorage.setItem('providerNextStep', result.provider.nextStep)
      }
      ...
      } else if (error.message.includes('especialidade')) {
        errorMessage = 'Por favor, selecione pelo menos uma especialidade.'
      } else if (error.message.includes('termos')) {
        errorMessage = 'Você deve aceitar os termos de uso para continuar.'
      } else if (error.message.includes('OAB')) {
        errorMessage = 'Já existe um cadastro com essa OAB. Tente fazer login.'
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Select value={formData.oabState} onValueChange={(value) => handleInputChange('oabState', value)}>
                    <SelectTrigger className="w-20" data-testid="oab-state-select">
        ...
        <div className="flex justify-between mt-6">
          ...
          {currentStep < STEPS.length ? (
            ...
            <LoadingButton
              ...
              loadingKey="provider-application-submit"
            >
              Enviar Aplicação
